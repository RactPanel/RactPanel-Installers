...
# Official RactPanel Automated Installation Script
# =================================================
#
...
# CentOS 7.*/8.* Minimal, 
# Ubuntu server 14.04/19.10
# Debian 7.*/10.*
# 32bit and 64bit
#
# Contributions from:
#
#   Pascal Peyremorte (ppeyremorte@ractpanel.org)
#   Mehdi Blagui 
#   Kevin Andrews (kevin@zvps.uk)
#   Mahmudul Hasan Rubel (bd.mhrubel@gmail.com)
#
#   and all those who participated to this and to previous installers.
#   Thanks to all.

## 
# RACTPANEL_CORE/INSTALLER_VERSION
# master - latest unstable
# 1.0.0 - example stable tag
##
RACTPANEL_INSTALLER_VERSION="master"
RACTPANEL_CORE_VERSION="1.0.0"

PANEL_PATH="/etc/ractpanel"
PANEL_DATA="/var/ractpanel"
PANEL_UPGRADE=false

#--- Display the 'welcome' splash/user warning info..
echo ""
echo "###############################################################"
echo "#  Welcome to the Official RactPanel Installer $RACTPANEL_INSTALLER_VERSION  #"
echo "###############################################################"
...
if [[ "$OS" = "CentOs" && ("$VER" = "7" || "$VER" = "8" ) || 
      "$OS" = "Ubuntu" && ("$VER" = "14.04" || "$VER" = "19.10" ) || 
      "$OS" = "debian" && ("$VER" = "8" || "$VER" = "10" ) ]] ; then
    echo "Ok."
else
    echo "Sorry, this OS is not supported by RactPanel." 
    exit 1
fi
...
if [[ "$OS" = "CentOs" ]] ; then
    if [[ "$ARCH" == "i386" || "$ARCH" == "i486" || "$ARCH" == "i586" || "$ARCH" == "i686" ]]; then
        ...
        echo " think it may work on your, please report it to the RactPanel forum or bugtracker."
        exit 1
    fi
fi
...
# Check for some common control panels that we know will affect the installation/operating of RactPanel.
if [ -e /usr/local/cpanel ] || [ -e /usr/local/directadmin ] || [ -e /usr/local/solusvm/www ] || [ -e /usr/local/home/admispconfig ] || [ -e /usr/local/lxlabs/kloxo ] ; then
    ...
    echo "is designed to install and configure RactPanel on a clean OS installation only."
    echo -e "\nPlease re-install your OS before attempting to install using this script."
    exit 1
fi

# Check for some common packages that we know will affect the installation/operating of RactPanel.
if [[ "$OS" = "CentOs" ]] ; then
...
# only check for ractpanel installed systems zpanel can now upgrade using this script
if [ -L "/etc/zpanel" ] && [ -d "/etc/zpanel"  ]; then
    ...
    if [ $pkginst = "y" ]; then
        ...
        echo "$pkginstlist"
        echo "This installer is designed to install and configure RactPanel on a clean OS installation only!"
        echo -e "\nPlease re-install your OS before attempting to install using this script."
        exit 1
    fi
    unset pkginst
...
extern_ip="$(wget -qO- http://api.ractpanel.org/ip.txt)"
...
#   -t <timezone-string>
# like :
#   ractpanel_install.sh -t Europe/Paris -d panel.domain.tld -i xxx.xxx.xxx.xxx
# notes:
...
if [[ "$PANEL_FQDN" == "" ]] ; then
    ...
    echo ' 1) the sub-domain that you want to use to access RactPanel panel,'
    ...
    while true; do
        echo ""
        read -e -p "Enter the sub-domain you want to access RactPanel panel: " -i "$PANEL_FQDN" PANEL_FQDN
        ...
        if [[ "$dns_panel_ip" == "" ]]; then
            ...
            echo "  For more information, read the RactPanel documentation:"
            echo "   - http://docs.ractpanel.org/index.php?node=7 (Installing RactPanel)"
            echo "   - http://docs.ractpanel.org/index.php?node=51 (Installer questions)"
            ...
            if [[ "$dns_panel_ip" != "$PUBLIC_IP" ]]; then
                ...
                echo "  RactPanel will not be reachable from http://$PANEL_FQDN"
                confirm="true"
            fi
        fi
        ...
        if [[ "$PUBLIC_IP" != "$extern_ip" && "$PUBLIC_IP" != "$local_ip" ]]; then
            ...
            echo "  RactPanel will not work with this IP..."
                confirm="true"
        fi
        ...
        if [[ "$confirm" != "" ]] ; then
            ...
            echo "Are you really sure that you want to setup RactPanel with these parameters?"
            ...
            read -e -p "All is ok. Do you want to install RactPanel now (y/n)? " yn
            case $yn in
                [Yy]* ) break;;
                [Nn]* ) exit;;
            esac
...
logfile=$(date +%Y-%m-%d_%H.%M.%S_ractpanel_install.log)
touch "$logfile"
exec > >(tee "$logfile")
exec 2>&1

echo "Installer version $RACTPANEL_INSTALLER_VERSION"
echo "RactPanel core version $RACTPANEL_CORE_VERSION"
echo ""
echo "Installing RactPanel $RACTPANEL_CORE_VERSION at http://$PANEL_FQDN and ip $PUBLIC_IP"
echo "on server under: $OS  $VER  $ARCH"
uname -a

# Function to disable a file by appending its name with _disabled
disable_file() {
    mv "$1" "$1_disabled_by_ractpanel" &> /dev/null
}
...
if [[ "$OS" = "CentOs" ]]; then
    ...
    # Removal of conflicting packages prior to RactPanel installation.
...
#--- Install utility packages required by the installer and/or RactPanel.
...
#--- Download RactPanel archive from GitHub
echo -e "\n-- Downloading RactPanel, Please wait, this may take several minutes, the installer will continue after this is complete!"
# Get latest ractpanel
while true; do
    wget -nv -O ractpanel_core.zip https://github.com/racthosting/ractpanel-core/archive/$RACTPANEL_CORE_VERSION.zip
    if [[ -f ractpanel_core.zip ]]; then
        break;
    else
        echo "Failed to download ractpanel core from Github"
...
###
# RactPanel Core Install 
###
mkdir -p $PANEL_PATH
mkdir -p $PANEL_DATA
chown -R root:root $PANEL_PATH
unzip -oq ractpanel_core.zip -d $PANEL_PATH
...
if [ ! -L "/etc/zpanel" ] && [ -d "/etc/zpanel" ]; then
    ...
    echo -e "Upgrading ZPanelCP 10.1.0 to RactPanel 1.0.0";

    PANEL_UPGRADE=true

    mv /etc/zpanel/configs /root/zpanel_configs_backup

    ## Move main directories to new ractpanel location ##
    ...
    rm -f /var/ractpanel/sieve/globalfilter.sieve
    rm -f /etc/dovecot/dovecot.conf
    rm -f /etc/proftpd.conf

    mysqlpassword=$(cat /etc/ractpanel/panel/cnf/db.php | grep "pass" | cut -d \' -f 2);

    ## Do NOT copy the new cnf directory
    rm -rf "$PANEL_PATH/ractpanel-core-$RACTPANEL_CORE_VERSION/cnf"
 
fi

## cp can be aliased to stop overwriting of files in centos use full path to cp
/bin/cp -rf "$PANEL_PATH/ractpanel-core-$RACTPANEL_CORE_VERSION/." "$PANEL_PATH/panel/"
rm ractpanel_core.zip
...
#--- Set-up RactPanel directories and configure permissions
...
#--- Install preconfig
while true; do
    wget -nv -O ractpanel_preconfig.zip https://github.com/ractpanel/ractpanel-installers/archive/$RACTPANEL_INSTALLER_VERSION.zip
    if [[ -f ractpanel_preconfig.zip ]]; then
        break;
    else
        echo "Failed to download ractpanel preconfig from Github"
        echo "If you quit now, you can run again the installer later."
        read -e -p "Press r to retry or q to quit the installer? " resp
        case $resp in
            [Rr]* ) continue;;
...
unzip -oq ractpanel_preconfig.zip
/bin/cp -rf ractpanel-installers-$RACTPANEL_INSTALLER_VERSION/preconf/* $PANEL_CONF
rm ractpanel_preconfig*
rm -rf ractpanel-*
...
#-----------------------------------------------------------
# Install all softwares and dependencies required by RactPanel.
...
# setup ractpanel access and core database
if [ $PANEL_UPGRADE == true ]; then

    mysql -u root -p"$mysqlpassword" < $PANEL_CONF/ractpanel-update/zpanel/sql/update-structure.sql
    mysql -u root -p"$mysqlpassword" < $PANEL_CONF/ractpanel-update/zpanel/sql/update-data.sql
    
    mysqldump -u root -p"$mysqlpassword" zpanel_core | mysql -u root -p"$mysqlpassword" -D ractpanel_core
    mysqldump -u root -p"$mysqlpassword" zpanel_postfix | mysql -u root -p"$mysqlpassword" -D ractpanel_postfix
    mysqldump -u root -p"$mysqlpassword" zpanel_proftpd | mysql -u root -p"$mysqlpassword" -D ractpanel_proftpd
    mysqldump -u root -p"$mysqlpassword" zpanel_roundcube | mysql -u root -p"$mysqlpassword" -D ractpanel_roundcube

    sed -i "s|zpanel_core|ractpanel_core|" $PANEL_PATH/panel/cnf/db.php

else
    sed -i "s|YOUR_ROOT_MYSQL_PASSWORD|$mysqlpassword|" $PANEL_PATH/panel/cnf/db.php
    mysql -u root -p"$mysqlpassword" < $PANEL_CONF/ractpanel-install/sql/ractpanel_core.sql
fi
...
if [ $PANEL_UPGRADE == false ]; then
    mysql -u root -p"$mysqlpassword" < $PANEL_CONF/ractpanel-install/sql/ractpanel_postfix.sql
fi

## grant will also create users which don't exist and update existing users with password ##
mysql -u root -p"$mysqlpassword" -e "GRANT ALL PRIVILEGES ON ractpanel_postfix .* TO 'postfix'@'localhost' identified by '$postfixpassword';";
...
mysql -u root -p"$mysqlpassword" -e "UPDATE ractpanel_core.x_settings SET so_value_tx='$HTTP_SERVICE' WHERE so_name_vc='httpd_exe'"
mysql -u root -p"$mysqlpassword" -e "UPDATE ractpanel_core.x_settings SET so_value_tx='$HTTP_SERVICE' WHERE so_name_vc='apache_sn'"
...
# remove default virtual site to ensure RactPanel is the default vhost
if [[ "$OS" = "CentOs" ]]; then
...
if [[ "$OS" = "CentOs" ]]; then
    sed -i "s|^\(NameVirtualHost .*$\)|#\1\n# NameVirtualHost is now handled in RactPanel vhosts file|" "$HTTP_CONF_PATH"
    sed -i 's|^\(Listen .*$\)|#\1\n# Listen is now handled in RactPanel vhosts file|' "$HTTP_CONF_PATH"
elif [[ "$OS" = "Ubuntu" || "$OS" = "debian" ]]; then
    sed -i "s|\(Include ports.conf\)|#\1\n# Ports are now handled in RactPanel vhosts file|" "$HTTP_CONF_PATH"
...
# Build suhosin for PHP 5.x which is required by RactPanel. 
...
if [ $PANEL_UPGRADE == false ]; then
    mysql -u root -p"$mysqlpassword" < $PANEL_CONF/ractpanel-install/sql/ractpanel_proftpd.sql
fi
...
mysql -u root -p"$mysqlpassword" -e "GRANT ALL PRIVILEGES ON ractpanel_proftpd .* TO 'proftpd'@'localhost' identified by '$proftpdpassword';";
...
mysql -u root -p"$mysqlpassword" -e "ALTER TABLE ractpanel_proftpd.ftpuser ALTER COLUMN uid SET DEFAULT $HTTP_UID"
mysql -u root -p"$mysqlpassword" -e "ALTER TABLE ractpanel_proftpd.ftpuser ALTER COLUMN gid SET DEFAULT $HTTP_GID"
sed -i "s|!SQL_MIN_ID!|$HTTP_UID|" $PANEL_CONF/proftpd/proftpd-mysql.conf

# Setup proftpd base file to call ractpanel config
rm -f "$FTP_CONF_PATH"
#touch "$FTP_CONF_PATH"
...
if [[ "$OS" = "CentOs" ]]; then
    ...
    BIND_SERVICE="bind9"
    BIND_USER="bind"
    mysql -u root -p"$mysqlpassword" -e "UPDATE ractpanel_core.x_settings SET so_value_tx='' WHERE so_name_vc='bind_log'"
fi
mysql -u root -p"$mysqlpassword" -e "UPDATE ractpanel_core.x_settings SET so_value_tx='$BIND_PATH' WHERE so_name_vc='bind_dir'"
mysql -u root -p"$mysqlpassword" -e "UPDATE ractpanel_core.x_settings SET so_value_tx='$BIND_SERVICE' WHERE so_name_vc='bind_service'"
...
mysql -u root -p"$mysqlpassword" -e "UPDATE ractpanel_core.x_settings SET so_value_tx='$CRON_FILE' WHERE so_name_vc='cron_file'"
mysql -u root -p"$mysqlpassword" -e "UPDATE ractpanel_core.x_settings SET so_value_tx='$CRON_FILE' WHERE so_name_vc='cron_reload_path'"
mysql -u root -p"$mysqlpassword" -e "UPDATE ractpanel_core.x_settings SET so_value_tx='$CRON_USER' WHERE so_name_vc='cron_reload_user'"
{
    echo "SHELL=/bin/bash"
...
sed -i "s|\$cfg\['blowfish_secret'\] \= 'RACTPANEL';|\$cfg\['blowfish_secret'\] \= '$phpmyadminsecret';|" $PANEL_CONF/phpmyadmin/config.inc.php
...
if [ $PANEL_UPGRADE == false ]; then
    mysql -u root -p"$mysqlpassword" < $PANEL_CONF/ractpanel-install/sql/ractpanel_roundcube.sql
fi
...
mysql -u root -p"$mysqlpassword" -e "GRANT ALL PRIVILEGES ON ractpanel_roundcube .* TO 'roundcube'@'localhost' identified by '$roundcubepassword';";
...
#--- Set some RactPanel database entries using. setso and setzadmin (require PHP)
echo -e "\n-- Configuring RactPanel"
zadminpassword=$(passwordgen);
setzadmin --set "$zadminpassword";
$PANEL_PATH/panel/bin/setso --set ractpanel_domain "$PANEL_FQDN"
$PANEL_PATH/panel/bin/setso --set server_ip "$PUBLIC_IP"

# if not release, set beta version in database
if [[ $(echo "$RACTPANEL_CORE_VERSION" | sed  's|.*-\(beta\).*$|\1|') = "beta"  ]] ; then
    $PANEL_PATH/panel/bin/setso --set dbversion "$RACTPANEL_CORE_VERSION"
fi
...
#--- Advise the admin that RactPanel is now installed and accessible.
{
echo "########################################################"
echo " Congratulations RactPanel has now been installed on your"
echo " server. Please review the log file left in /root/ for "
echo " any errors encountered during installation."
echo ""
echo " Login to RactPanel at http://$PANEL_FQDN"
echo " RactPanel Username  : zadmin"
echo " RactPanel Password  : $zadminpassword"
echo ""
echo " MySQL Root Password      : $mysqlpassword"
echo " MySQL Postfix Password   : $postfixpassword"
echo " MySQL ProFTPd Password   : $proftpdpassword"
